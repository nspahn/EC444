...
int writeRegister(uint8_t reg, uint8_t data) {
  int ret; // reuse var name from previous funcitons
  i2c_cmd_handle_t cmd = i2c_cmd_link_create();//from https://esp32.com/viewtopic.php?t=11760
  i2c_master_start(cmd);
  ...
  i2c_cmd_link_delete(cmd);
  if (ret == ESP_FAIL)
		  printf("writeRegister has failed.\n");
  return ret;
}
...
uint8_t readRegister(uint8_t reg) {
  ...
  if (ret == ESP_FAIL)
			printf("readRegister has failed.\n");
...
int16_t read16(uint8_t reg) {
  uint8_t data1, data2;
  int ret; //unused if included but not initialised if not included?
...
void calcRP(float x, float y, float z){
  // Code from https://wiki.dfrobot.com/How_to_Use_a_Three-Axis_Accelerometer_for_Tilt_Sensing.
	  double roll = 0.00;
	  double pitch = 0.00;
