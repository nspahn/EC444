...
int len_out = 4;
int current_state = nonleader; // Variable to control current state
bool election = false, leader_present = true;
...
static void uart_init() {
  ...
  uart_set_line_inverse(UART_NUM_1, UART_SIGNAL_RXD_INV); // Changed from UART_INVERSE_RXD
    ...
    if (len_in >0) {
      if (data_in[0] == start) {
        if (checkCheckSum(data_in,len_out)) {
		  ...
		  if(data_in[1] == 1)
			leader_present = true;
		  if( (data_in[1] == 1 && current_state == leader) && (myID < data_in[2])){
			printf("Okay, I am not the leader\n");
			current_state = nonleader;
			leader_present = true;
			election = false;
			free(data_in);
			return;
		...
		//printf("Didn't pass the previous thing :(\n");
      }
    }
    else{
...
void send_task(){
  ...
  while(1) {
	...
	if(election) {
		printf("We are having an election\n");
		recv_task();
		if(election) // Check for another response
    ...
    data_out[0] = start;
    data_out[1] = (char)(current_state); // Send a character
